...
class ScaleLookup:
    ...
    def resolve(self, image_path: Path) -> float:
        return get_scale_um_per_px(
            image_path=image_path,
            default_scale_value=self.default_value,
            scale_keys=list(self.keys),
            scale_values=list(self.values),
        )
...
def find_tif_images_by_keys(
    ...
    max_subjects : Optional[int]
        Maximum number of subjects to return (paired + unmatched when include_unmatched=True)
    include_unmatched : bool
        When True, return single-channel images that lack key matches (treated as combined channels).
        Disable to skip such images (useful when directories contain extra channels like c1).
...
def run_dask_pipeline(
    ...
    *,
    ...
    force_respawn_cluster: bool = False,
    debug: bool = True,
    max_subjects: Optional[int] = None,
    blob_size_um: Optional[float] = None,
    connect_to_existing_cluster: bool = False,
    use_timestamps: Optional[bool] = False,
    debug_image_capture: bool = True,
    debug_image_whitelist: Optional[Sequence[str]] = None,
    debug_subject_limit: Optional[int] = None,
    ...
    effective_blob_size = float(blob_size_um) if blob_size_um is not None else float(dataset_cfg.blob_size_um)
                    ...
                    try:
                        ...
                        if not params_match:
                            ...
                            if force_respawn_cluster:
                                print("    force_respawn_cluster=True: Closing existing cluster and respawning...")
                                needs_respawn = True
                                existing_client.close()
                                existing_client = None
                            else:
                                print("    Using existing cluster anyway (set force_respawn_cluster=True to respawn)")
                        else:
    ...
    results_dir: Path = setup_results_dir(results_root, exp_name=dataset_cfg.exp_name)
    ...
    if debug_image_capture:
        whitelist = list(debug_image_whitelist) if debug_image_whitelist is not None else list(DEFAULT_DEBUG_STAGE_WHITELIST)
    ...
    def _segment_both(rfp, dapi, mpp, debug_session=None):
        base, best = segment_crypts_dual(
            channels=(rfp, dapi),
            microns_per_px=mpp,
            blob_size_um=effective_blob_size,
            debug=False,
            max_regions_best=dataset_cfg.max_regions_per_image,
