before any discussion of the pipeline, i should show the images that we are workign with, a couple examples, state the target, dont need to go into to much biological background yet


then we go to N1, thsi would just be something that they can glance over, we should probobly simplify it down to the curent step 1, and then build morphology maps for dapi and RFP, combine maps to predict crypt locations, then label individual crypt regions,  then the apply weighted quality scoring and selection, then the text on the bottom is good 
( tho mods, increase text size in bubble add dial, make bubbles that red color)
