...
def remove_rectangular_artifacts(
    *,
    ...
    # Remember original shape to restore after processing
    original_shape = image.shape
    
    # Squeeze out any leading singleton dimensions (e.g., from dask/xarray groupby)
    image_squeezed = np.squeeze(image)
    
    if image_squeezed.ndim < 2:
        raise ValueError(f"Artifact removal requires at least 2D image data, got shape {image.shape} (squeezed: {image_squeezed.shape})")

    original_dtype = image_squeezed.dtype
    image_uint8 = image_squeezed.astype(np.uint8, copy=False)
    ...
    if original_dtype != np.uint8:
        cleaned = cleaned.astype(original_dtype, copy=False)
    
    # Restore original shape by reshaping if necessary
    if original_shape != cleaned.shape:
        # Add back any singleton dimensions that were squeezed
