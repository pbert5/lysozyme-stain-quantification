...
USE_RFP_DAPI_PROCESSING = True  # Set to True to use RFP/DAPI watershed processing
project_root = Path(__file__).parent.parent.parent

if USE_RFP_DAPI_PROCESSING:
    print("Using RFP/DAPI watershed processing...")
    processor = RFPDAPIProcessor(project_root=project_root, debug=True)
    results = processor.process_all_pairs(max_pairs=30, show_plots=False)
    print(f"Processed {len(results)} RFP/DAPI pairs successfully")
else:
    print("Using traditional blob processing...")
    # 1) Define your inputs:
    img_dir = Path("lysozyme images/Jej LYZ")       # ← change this
